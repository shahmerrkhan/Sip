...
import { db } from '@/db';
import { sipNotes, requests, seekers } from '@/db/schema';
...
export async function GET(req: Request) {
  ...
  const result = await db.select().from(sipNotes).where(eq(sipNotes.mentorId, mentorId)).orderBy(desc(sipNotes.createdAt));
...
export async function POST(req: Request) {
  ...
  const created = await db.insert(sipNotes).values({ mentorId, seekerName, seekerEmail, note }).returning();
