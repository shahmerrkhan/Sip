...
export default function SeekerProfile() {
  ...
  if (!seeker) return (
    <div style={{ background: '#0D1117', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
      ...
      <p style={{ color: '#8B949E' }}>Seeker not found.</p>
    ...
    <div style={{ background: '#0D1117', minHeight: '100vh', color: '#E6EDF3' }}>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '90px 16px 60px' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
          style={{ background: '#161B22', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 24, padding: 'clamp(20px,5vw,40px)', marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 20, marginBottom: 28 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              {seeker.linkedin && (
                <a href={seeker.linkedin.startsWith('http') ? seeker.linkedin : `https://${seeker.linkedin}`} target="_blank" rel="noopener noreferrer" style={{ color: '#70B5F9', textDecoration: 'none', fontSize: 14 }}>LinkedIn ↗</a>
              )}
            </div>
          </div>
          ...
          {interests.length > 0 && (
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 28 }}>
              {interests.map(i => (
                <span key={i} style={{ background: 'rgba(112,181,249,0.07)', border: '1px solid rgba(112,181,249,0.15)', color: '#70B5F9', padding: '5px 14px', borderRadius: 14, fontSize: 13 }}>{i}</span>
              ))}
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
            <div style={{ background: '#0D1117', borderRadius: 12, padding: '16px', textAlign: 'center' }}>
              ...
              <div style={{ color: '#8B949E', fontSize: 12, marginTop: 4 }}>Week Streak</div>
            </div>
            <div style={{ background: '#0D1117', borderRadius: 12, padding: '16px', textAlign: 'center' }}>
              <div style={{ fontSize: 24, fontWeight: 700, color: '#70B5F9', fontFamily: 'Space Mono' }}>{seeker.sips.length}</div>
              <div style={{ color: '#8B949E', fontSize: 12, marginTop: 4 }}>Sips Shared</div>
            </div>
          </div>
        </motion.div>
        ...
        {seeker.sips.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.5 }}
            style={{ background: '#161B22', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 24, padding: 'clamp(20px,5vw,40px)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {seeker.sips.map(s => (
                <Link key={s.mentorId} href={`/mentors/${s.mentorId}`} style={{ textDecoration: 'none' }}>
                  <div style={{ background: '#0D1117', borderRadius: 14, padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <div style={{ color: '#E6EDF3', fontSize: 15, fontWeight: 600 }}>{s.firstName} {s.lastName}</div>
                      <div style={{ color: '#8B949E', fontSize: 13 }}>{s.role} @ {s.company}</div>
                    </div>
                    <span style={{ color: '#70B5F9', fontSize: 13 }}>view →</span>
                  </div>
                </Link>
              ))}
            </div>
