...
export default function Home() {
  ...
  return (
      ...
      <motion.nav
        ...
        <motion.span whileHover={{ scale: 1.05 }} style={{ fontFamily: 'Space Mono', fontSize: 22, fontWeight: 700, color: '#70B5F9', letterSpacing: -1, cursor: 'default' }}>sip ☕</motion.span>
        {!user && (
        ...
        <motion.div
          ...
          {user ? (
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }} style={{ display: 'flex', gap: 12 }}>
              {rolesLoaded && isMentor && <Link href="/dashboard" style={{ display: 'block', background: '#0A66C2', color: 'white', border: 'none', padding: '14px 30px', borderRadius: 28, fontSize: 15, fontWeight: 600, textDecoration: 'none', fontFamily: 'inherit' }}>go to my dashboard →</Link>}
              {rolesLoaded && isSeeker && (
                <>
                  ...
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }} onClick={handleInstantSip} style={{ background: 'transparent', color: '#5BDB8A', border: '1px solid rgba(91,219,138,0.3)', padding: '14px 30px', borderRadius: 28, fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>⚡ instant sip</motion.button>
                </>
              )}
...
fontFamily: 'inherit', transition: 'background 0.2s' }}>
      ...
      {/* FOOTER */}
      <footer style={{ borderTop:'1px solid rgba(255,255,255,0.06)', padding: '32px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: '#8B949E', fontSize: 13 }}>
        <span style={{ fontFamily: 'Space Mono', color: '#70B5F9', fontWeight: 700 }}>sip ☕</span>
        <span>built for the curious ones</span>
        <Link href="/mentors/signup" style={{ color: '#70B5F9', textDecoration: 'none' }}>are you a mentor? →</Link>
      </footer>
    </div>
  );
}